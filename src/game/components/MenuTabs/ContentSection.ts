...
export interface ImageOpts extends LayoutOpts {
    ...
    /**
     * When true, a red debug border is drawn around the image's display bounds (for layout debugging).
     */
    debugRedBorder?: boolean;
}
...
export interface RowOpts extends LayoutOpts {
    ...
    gap?: number;
    /**
     * Optional per-item width percentages (same semantics as Grid columnWidthPercents).
     * When provided, each row item gets a share of the available width (after gaps) according to these values.
     * If fewer values are provided than items, remaining items share the leftover percentage equally.
     * Values are normalized so total width fits the row. E.g. [30] with 2 items → [30%, 70%].
     */
    columnWidthPercents?: number[];
}
...
export interface ContentSection {
    ...
    margin?: Spacing;
    /** When true, a red debug border is drawn around the section border (e.g. scatter payout). */
    debugRedBorder?: boolean;
    /** When true, a red debug border is drawn around each content item (Header, Image, Grid, Text) inside the section. */
    debugRedBordersOnElements?: boolean;
}
